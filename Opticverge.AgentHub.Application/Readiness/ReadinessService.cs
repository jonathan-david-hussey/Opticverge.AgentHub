...
public sealed class ReadinessService(TimeProvider timeProvider)
{
    public ReadinessSummary SummarizeRepository(string repositoryName)
    {
        ...
        [
            ...
            new("boundaries", "Architecture boundaries are visible", ReadinessCheckStatus.Passed,
                "Domain, Application, Infrastructure, Web, API, Worker, MCP, and Evaluation projects are separated.", 15),
            new("observability", "Telemetry is configured", ReadinessCheckStatus.Passed,
                "ServiceDefaults enables OpenTelemetry logs, metrics, traces, health, and service discovery.", 15),
            new("secrets", "Secrets are externalized", ReadinessCheckStatus.Passed,
                "Provider credentials and identity secrets are expected through user secrets or environment variables.", 10),
            new("runtime-diagnostics", "Runtime diagnostics are available", ReadinessCheckStatus.Warning,
                "Aspire dashboard, Seq, Prometheus, Grafana, and MCP are scaffolded; full persisted diagnostics are future work.", 10)
        ];
